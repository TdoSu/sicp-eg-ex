...
;;; 过程作为黑箱抽象
;;; 用 square, good-enough?, average 这些名字把具体实现抽取出来之后,
;;; 使用这些名字时, 我们只需要关心输入和输出,
;;; 不需要关系内部实现, 后面我们也可以方便的替换内部实现.
;;; 这些名字不在仅仅指某一个内部实现, 而是表示了满足这个输入输出的所有过程,
;;; 这就是 "抽象" 的意思.
;;; 就像 "苹果" 并不是某一个苹果.

;;; 局部名
;;; (define square (lambda (x) (* x x)))
;;; 在 lambda 表达式中 x 可以被统一替换为任何一个表达式中没用到的符号,
;;; 而不会改变这个表达式.
;;; 这时这个表达式叫做 x 的作用域, x 在这个表达式中是约束的, 是一个局部名.
;;; 而 * 这个符号, 在这个 lambda 表达式中是自由的.

;;; 块结构
;;; 在过程内部定义的过程, 叫做块结构.
;;; 内部过程的符号可以被外部过程约束.
;;; 比如 sqrt 里面定义 good-enough?,
;;; good-enough? 中如果没约束 x, 那么就会被 sqrt 的 x 约束.
;;; 这叫做词法作用域.(在词法分析阶段就可以确定约束关系)
