    ...
    (define (set-my-value newval setter)
      (cond ((not (has-value? me))
             ...
             (for-each-except setter
                              ...
                              constraints))
            ((not (= value newval))
             (error 'MAKE-CONNECTOR "Contradiction" (list value newval)))
            (else 'ignored)))
...
; (set-value! a 2 'user)
; Error! Contradiction with irritant (12 2)
...
; Probe: b = 9
; a 没有触发更新
; 因为对于 multiplier 的 process-new-value 来说
; b --> product 有值, 而 a --> m1 m2 都没有值...
; a b 如果同时设置值, 就会冲突
; done
